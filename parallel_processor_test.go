...
func TestParallelProcessor(t *testing.T) {
	// 创建增强资源管理器
	resourceManager := NewEnhancedResourceManager()
	defer resourceManager.Shutdown()

	// 创建并行处理器
	processor := NewParallelProcessor(resourceManager)
...
func testProcessSingleVideo(t *testing.T, processor *ParallelProcessor, videoPath string) {
...
func testProcessBatch(t *testing.T, processor *ParallelProcessor, videos []string) {
...
func testStatusQueries(t *testing.T, processor *ParallelProcessor) {
	// 获取处理器状态
...
func TestParallelProcessorIntegration(t *testing.T) {
	...
	// 启动多个处理任务
	pipelines := make([]*ProcessingPipeline, 0)
