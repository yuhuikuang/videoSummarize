...
func TestTextCorrection(t *testing.T) {
	...
	testDir := filepath.Join(dataRoot(), testJobID)
	...
	// 执行文本修正
	correctedSegments, correctionSession, err := correctTranscriptSegments(testSegments, testJobID)
	if err != nil {
	...
	// 保存修正会话记录
	if err := saveCorrectionSession(testDir, correctionSession); err != nil {
		t.Errorf("Failed to save correction session: %v", err)
	}

	// 保存修正后的转录文件
	if err := saveCorrectedTranscript(testDir, correctedSegments); err != nil {
...
func TestTextCorrectionIntegration(t *testing.T) {
	...
	correctedSegments, err := transcribeAudio("", testJobID) // 空音频路径，因为我们已经有转录结果
	if err != nil {
