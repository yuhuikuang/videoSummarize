...
type HybridSearchResult struct {
	VectorHits    []Hit     `json:"vector_hits"`
	FullTextHits  []Hit     `json:"fulltext_hits"`
	CombinedHits  []Hit     `json:"combined_hits"`
...
// HybridSearch 混合搜索
func (s *EnhancedVectorStore) HybridSearch(jobID, query string, topK int) ([]Hit, error) {
	// 使用默认平衡策略
...
func (s *EnhancedVectorStore) HybridSearchWithStrategy(jobID, query string, topK int, strategy HybridSearchStrategy) ([]Hit, error) {
	// 获取查询向量
...
// vectorSearch 向量搜索
func (s *EnhancedVectorStore) vectorSearch(jobID, query string, topK int) []Hit {
	// 使用原有的向量搜索逻辑
	return s.Search(jobID, query, topK)
}

// fullTextSearch 全文搜索
func (s *EnhancedVectorStore) fullTextSearch(jobID, query string, topK int) ([]Hit, error) {
	if s.videoID == "" {
		s.videoID = jobID
	}
...
func (s *EnhancedVectorStore) combineAndRerankResults(vectorHits, fullTextHits []Hit, query string, topK int) []Hit {
	// 创建结果映射，避免重复
	resultMap := make(map[string]Hit)
	...
	// 转换为切片并排序
	combined := make([]Hit, 0, len(resultMap))
...
func (s *EnhancedVectorStore) calculateAverageScore(hits []Hit) float64 {
	if len(hits) == 0 {
		return 0
	}
...
func (s *EnhancedVectorStore) Upsert(jobID string, items []Item) int {
	if len(items) == 0 {
		return 0
	}
...
func (s *EnhancedVectorStore) Search(jobID string, query string, topK int) []Hit {
	if topK <= 0 {
		topK = 5
	}
	if s.videoID == "" {
...
func (s *EnhancedVectorStore) OptimizedBatchUpsert(jobID string, items []Item) error {
	if len(items) == 0 {
		return nil
	}
