...
// JobResource 已在 models.go 中统一定义

// JobRequest 已在 models.go 中统一定义
...
// PerformanceMetrics 已在 models.go 中统一定义
...
func (rm *ResourceManager) collectPerformanceMetrics() {
	// 更新系统资源使用情况
	rm.resourceMu.RLock()
	rm.performanceMetrics.CPUUsage = rm.cpuUsage
	rm.performanceMetrics.MemoryUsage = rm.memoryUsed
	if rm.gpuAvailable {
		rm.performanceMetrics.GPUUsage = float64(rm.gpuMemoryUsed) / float64(rm.gpuMemoryTotal) * 100
	}
	rm.resourceMu.RUnlock()
	
	// 更新时间戳
	rm.performanceMetrics.EndTime = time.Now()
	rm.performanceMetrics.Duration = rm.performanceMetrics.EndTime.Sub(rm.performanceMetrics.StartTime)
	
	log.Printf("Performance metrics: cpu=%.2f%%, memory=%dMB, gpu=%.2f%%, duration=%v",
		rm.performanceMetrics.CPUUsage,
		rm.performanceMetrics.MemoryUsage,
		rm.performanceMetrics.GPUUsage,
		rm.performanceMetrics.Duration)
}
...
func (rm *ResourceManager) updatePerformanceMetrics(action string, job *JobResource) {
	...
	case "completed":
		if job != nil {
			...
			// 更新持续时间
			rm.performanceMetrics.Duration = duration
			// 计算吞吐量 (作业/秒)
			if duration.Seconds() > 0 {
				rm.performanceMetrics.Throughput = 1.0 / duration.Seconds()
			}
		}
	case "failed":
		rm.performanceMetrics.ErrorCount++
	case "released":
		// 作业释放时的处理
	}
}
