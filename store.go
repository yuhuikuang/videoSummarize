...
type VectorStore interface {
	Upsert(jobID string, items []Item) int
	Search(jobID string, query string, topK int) []Hit
}
...
func (s *MemoryVectorStore) Upsert(jobID string, items []Item) int {
...
func (s *MemoryVectorStore) Search(jobID string, query string, topK int) []Hit {
	...
	for _, sc := range scores[:topK] {
		d := docs[sc.i]
		hits = append(hits, Hit{Score: sc.score, Start: d.Start, End: d.End, Text: d.Text, Summary: d.Summary, FramePath: d.FramePath})
	}
	return hits
}
...
func (s *MilvusVectorStore) Upsert(jobID string, items []Item) int {
...
func (s *MilvusVectorStore) Search(jobID string, query string, topK int) []Hit {
	...
	for _, r := range res {
		...
		for i := 0; i < r.ResultCount; i++ {
			...
			hits = append(hits, Hit{Score: score, Start: start, End: end, Text: text, Summary: summary, FramePath: frame})
		}
	}
	return hits
}
...
func (s *PgVectorStore) Search(jobID string, query string, topK int) []Hit {
	if topK <= 0 { topK = 5 }
	if s.videoID == "" {
		fmt.Printf("Warning: video_id not set for PgVectorStore, using jobID as video_id\n")
		s.videoID = jobID
...
func storeHandler(w http.ResponseWriter, r *http.Request) {
	...
	items := req.Items
	if len(items) == 0 {
		b, err := os.ReadFile(filepath.Join(dataRoot(), req.JobID, "items.json"))
		if err != nil { writeJSON(w, http.StatusBadRequest, map[string]string{"error": "items missing and items.json not found"}); return }
		if err := json.Unmarshal(b, &items); err != nil { writeJSON(w, http.StatusBadRequest, map[string]string{"error": "invalid items.json"}); return }
	}
	cnt := globalStore.Upsert(req.JobID, items)
	writeJSON(w, http.StatusOK, StoreResponse{JobID: req.JobID, Count: cnt})
}
...
func queryHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil { writeJSON(w, http.StatusBadRequest, map[string]string{"error": "invalid json"}); return }
	if req.JobID == "" || strings.TrimSpace(req.Question) == "" { writeJSON(w, http.StatusBadRequest, map[string]string{"error": "job_id and question required"}); return }
	...
	hits := globalStore.Search(req.JobID, req.Question, req.TopK)
	ans := synthesizeAnswer(req.Question, hits)
	writeJSON(w, http.StatusOK, QueryResponse{JobID: req.JobID, Question: req.Question, Answer: ans, Hits: hits})
}
...
func storeItems(items []Item, jobID string) (int, error) {
	if globalStore == nil {
...
func synthesizeAnswer(question string, hits []Hit) string {
...
func synthesizeAnswerWithRAG(question string, hits []Hit) string {
	config, err := loadConfig()
...
func synthesizeAnswerSimple(question string, hits []Hit) string {
