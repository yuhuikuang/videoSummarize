...
func submitJobHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 提交作业
	err := enhancedResourceManager.SubmitJob(req.JobType, req.Priority, req.Payload, func(result *JobResult) {
...
func hybridSearchHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.Strategy != nil {
		// 使用指定策略
		results, err = enhancedVectorStore.HybridSearchWithStrategy(req.JobID, req.Query, req.TopK, *req.Strategy)
		strategyUsed = req.Strategy.Strategy
	} else {
		// 使用默认策略
		results, err = enhancedVectorStore.HybridSearch(req.JobID, req.Query, req.TopK)
...
func configHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		// 获取向量存储状态
		if enhancedVectorStore != nil {
			config["vector_store"] = enhancedVectorStore.GetStatus()
		}
...
func searchHandler(w http.ResponseWriter, r *http.Request) {
	...
	if enhancedVectorStore != nil {
		if req.Strategy != nil {
			// 使用指定策略
			hits, err = enhancedVectorStore.HybridSearchWithStrategy(req.JobID, req.Query, req.TopK, *req.Strategy)
			strategyUsed = req.Strategy.Strategy
		} else {
			// 使用默认策略
			hits, err = enhancedVectorStore.HybridSearch(req.JobID, req.Query, req.TopK)
			strategyUsed = "balanced"
		}
	} else {
		// 回退到传统搜索
